...
const GALLERY_COLLECTION = "gallery2_sections";
...
export default class GallerySection2 {
  constructor(rootId) {
  ...
  injectStyles() {
    if (document.getElementById("sierra-gallery2-style")) return;

    const style = document.createElement("style");
    style.id = "sierra-gallery2-style";
    style.textContent = `
      .sierra-gallery2-track {
        scrollbar-width: thin;
        scrollbar-color: rgba(79, 102, 85, 0.45) rgba(79, 102, 85, 0.12);
      }
      .sierra-gallery2-track::-webkit-scrollbar {
        height: 8px;
      }
      .sierra-gallery2-track::-webkit-scrollbar-thumb {
        background: rgba(79, 102, 85, 0.45);
        border-radius: 999px;
      }
      .sierra-gallery2-track::-webkit-scrollbar-track {
  ...
  render(title, items) {
    ...
    const heading = title || "Galerie 2";
    const cards = items.map((item) => `
      <article class="sierra-gallery2-card h-[50vh] overflow-hidden rounded-xl border border-[#4f6655]/25 bg-white shadow-sm">
    ...
    this.root.innerHTML = `
      <section class="mx-auto w-full max-w-7xl px-4 pb-6 pt-3 md:px-6 md:pb-8">
        <div class="mb-3 flex items-center justify-between">
          ...
          <span class="text-xs font-medium uppercase tracking-[0.08em] text-stone-500">Gallery 2</span>
        </div>
        ...
        <div class="relative">
          ...
          <div class="sierra-gallery2-track flex snap-x snap-mandatory gap-3 overflow-x-auto overflow-y-hidden pb-2 md:gap-4">
            ${cards}
          </div>
        </div>
      </section>
  ...
  bindNavControls() {
    const track = this.root.querySelector(".sierra-gallery2-track");
    if (!track) return;
